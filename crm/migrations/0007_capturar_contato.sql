-- capturar_contato: chamada quando o lead compartilha o contato (request_contact).
-- O /start<grupo> foi uma mensagem anterior, entao o codigo do grupo nao vem no
-- evento de contato -> recupera do ultimo 'start' desse telegram_id.
-- Merge inverso: casa o telegram_id com o lead do CRM (pelo telefone).
--   fundido | telefone_add | telegram_add | novo | ja_completo |
--   conflito_telegram | telefone_invalido
...
begin
  ...
  if v_tel_id is not null and v_tel_id = v_tg_id then
    ...
    v_lead := v_tg_id; v_status := 'telefone_add';
